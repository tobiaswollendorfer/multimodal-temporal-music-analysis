...
This adapted version keeps selected Spotify metadata columns in the final
assignment output for later analysis.

Objective:
    1. Select exactly N genres.
    2. Assign songs to candidate genres.
    3. Maximize temporal coverage:
       number of selected genre/modality/window cells with at least min_tracks
       songs.
    4. Use genre-score quality as a secondary objective.

Additional saved metadata columns:
    release, danceability, energy, key, mode, valence, tempo, duration_ms

Example:
    python src/01_create_dataset_approach3_ORTools_with_metadata.py \
        --total-genres 50 \
        --min-tracks 20 \
...
def load_track_metadata() -> pd.DataFrame:
    metadata = load_tsv(
        RAW_DIR / metadata_path,
        usecols=metadata_columns_to_keep,
    )
...
def enrich_assignments_with_metadata(
    ...
    enriched = assignments.merge(
        metadata[metadata_columns_to_keep],
        on="id",
        how="left",
    )
...
def create_genre_window_coverage(
    ...
    if "release" not in assigned.columns:
...
def summarize_assignment_metadata(assignments: pd.DataFrame) -> pd.DataFrame:
    return assignments[
        [
            "release",
            "danceability",
...
def main() -> None:
    ...
    type=int,
    default=42,
    help="Random seed for OR-Tools CP-SAT search.",
)

    args = parser.parse_args()

    output_dir = PROCESSED_DIR / "01_selected_genres_ortools"
    ...
    assignments = enrich_assignments_with_metadata(
    ...
    metadata_summary = summarize_assignment_metadata(assignments)
    ...
    assignments.to_csv(
        output_dir / f"{prefix}_assignments_with_metadata.csv",
        index=False,
    )
    ...
    metadata_summary.to_csv(
        output_dir / f"{prefix}_assignment_metadata_summary.csv",
        index=False,
    )
    ...
    print(summary)
    print()
    print("Assignment metadata summary:")
